...
struct Raster { float4 position [[position]]; float4 color; float distance; };
...
fragment float4 worldFragment(Raster in [[stage_in]]) {
    float fog = clamp((in.distance-4.0)/24.0, 0.0, 0.84);
    float3 color = mix(in.color.rgb, float3(0.026,0.038,0.053), fog);
